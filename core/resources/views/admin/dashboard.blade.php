...
@section('panel')
    ...
    <!--</div>-->
    @endif
    @if(auth()->guard('admin')->check() && auth()->guard('admin')->user()->isSubAdmin == 1)
    <div class="row mt-4">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body text-center">
                    <h5 class="card-title">@lang('Your Referral Code')</h5>
                    <p class="lead">@lang('Share this code to invite users:')</p>
                    <div class="input-group w-50 mx-auto">
                        <input type="text" class="form-control"
                        value="{{ url('/user/register/' . (auth()->guard('admin')->user()->invitation_code ?? 'Not Set')) }}"
                        readonly>
                        <span class="input-group-text bg--primary copyLink" style="cursor: pointer;">@lang('COPY')</span>
                    </div>
                </div>
            </div>
        </div>
