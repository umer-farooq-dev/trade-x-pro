...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if ($admin->isSubAdmin == 1) {
            $widget['total_users'] = User::where('ref_by', $admin->id)->count();
            $widget['verified_users'] = User::where('ref_by', $admin->id)->active()->count();
            $widget['email_unverified_users'] = User::where('ref_by', $admin->id)->emailUnverified()->count();
            $widget['mobile_unverified_users'] = User::where('ref_by', $admin->id)->mobileUnverified()->count();
            $referredUserIds = User::where('ref_by', $admin->id)->pluck('id');
            $widget['total_trade'] = TradeLog::whereIn('user_id', $referredUserIds)->count();
            $widget['total_wining_trade'] = TradeLog::whereIn('user_id', $referredUserIds)->win()->count();
            $widget['total_losing_trade'] = TradeLog::whereIn('user_id', $referredUserIds)->loss()->count();
            $widget['total_draw_trade'] = TradeLog::whereIn('user_id', $referredUserIds)->draw()->count();

            $widget['total_practice_trade'] = PracticeLog::whereIn('user_id', $referredUserIds)->count();
            $widget['total_practice_wining_trade'] = PracticeLog::whereIn('user_id', $referredUserIds)->win()->count();
            $widget['total_practice_losing_trade'] = PracticeLog::whereIn('user_id', $referredUserIds)->loss()->count();
            $widget['total_practice_draw_trade'] = PracticeLog::whereIn('user_id', $referredUserIds)->draw()->count();
            $userLoginData = UserLogin::whereIn('user_id', $referredUserIds)
                ->where('created_at', '>=', Carbon::now()->subDay(30))
                ->get(['browser', 'os', 'country']);
            $deposit['total_deposit_amount'] = Deposit::whereIn('user_id', $referredUserIds)->successful()->sum('amount');
            $deposit['total_deposit_pending'] = Deposit::whereIn('user_id', $referredUserIds)->pending()->count();
            $deposit['total_deposit_rejected'] = Deposit::whereIn('user_id', $referredUserIds)->rejected()->count();
            $deposit['total_deposit_charge'] = Deposit::whereIn('user_id', $referredUserIds)->successful()->sum('charge');
            $withdrawals['total_withdraw_amount'] = Withdrawal::whereIn('user_id', $referredUserIds)->approved()->sum('amount');
            $withdrawals['total_withdraw_pending'] = Withdrawal::whereIn('user_id', $referredUserIds)->pending()->count();
            $withdrawals['total_withdraw_rejected'] = Withdrawal::whereIn('user_id', $referredUserIds)->rejected()->count();
            $withdrawals['total_withdraw_charge'] = Withdrawal::whereIn('user_id', $referredUserIds)->approved()->sum('charge');
            $trxReport['date'] = collect([]);
            $plusTrx = Transaction::whereIn('user_id', $referredUserIds)
                ->where('trx_type', '+')
            ...
            $minusTrx = Transaction::whereIn('user_id', $referredUserIds)
                ->where('trx_type', '-')
                ->where('created_at', '>=', Carbon::now()->subDays(30))
                ->selectRaw("SUM(amount) as amount, DATE_FORMAT(created_at,'%Y-%m-%d') as date")
                ->orderBy('created_at')
            ...
            $depositsMonth = Deposit::whereIn('user_id', $referredUserIds)
            ...
            $withdrawalMonth = Withdrawal::whereIn('user_id', $referredUserIds)
                ->where('created_at', '>=', Carbon::now()->subYear())
                ->where('status', Status::PAYMENT_SUCCESS)
            ...
            $userLoginData = UserLogin::where('created_at', '>=', Carbon::now()->subDay(30))
                ->get(['browser', 'os', 'country']);
        ...
        $chart['user_browser_counter'] = $userLoginData->groupBy('browser')->map(function ($item) {
            return collect($item)->count();
        });
        $chart['user_os_counter'] = $userLoginData->groupBy('os')->map(function ($item) {
            return collect($item)->count();
        });
        $chart['user_country_counter'] = $userLoginData->groupBy('country')->map(function ($item) {
            return collect($item)->count();
        })->sort()->reverse()->take(5);
