...
class WithdrawalController extends Controller
{
    ...
    protected function withdrawalData($scope = null, $summery = false)
    {
        ...
        // Apply sub-admin filter if isSubAdmin == 1
        if ($admin->isSubAdmin == 1) {
            $withdrawals->whereHas('user', function ($query) use ($admin) {
        ...
        if (!$summery) {
            ...
            $successfulSummิสery = $successful->where('status', Status::PAYMENT_SUCCESS)->sum('amount');
    ...
    public function details($id)
    {
        ...
        if ($admin->isSubAdmin == 1 && $withdrawal->user->ref_by != $admin->id) {
            abort(403, 'Unauthorized: You can only view withdrawals of users you referred.');
        }
    ...
    public function approve(Request $request)
    {
        ...
        // Verify sub-admin access
        if ($admin->isSubAdmin == 1 && $withdraw->user->ref_by != $admin->id) {
            abort(403, 'Unauthorized: You can only approve withdrawals of users you referred.');
        }
    ...
    public function reject(Request $request)
    {
        ...
        // Verify sub-admin access
        if ($admin->isSubAdmin == 1 && $withdraw->user->ref_by != $admin->id) {
            abort(403, 'Unauthorized: You can only reject withdrawals of users you referred.');
        }
