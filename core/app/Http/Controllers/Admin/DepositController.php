...
class DepositController extends Controller
{
    ...
    protected function depositData($scope = null, $summery = false)
    {
        ...
        if ($admin->isSubAdmin == 1) {
            $deposits->whereHas('user', function ($query) use ($admin) {
    ...
    public function details($id)
    {
        ...
        if ($admin->isSubAdmin == 1 && $deposit->user->ref_by != $admin->id) {
            abort(403, 'Unauthorized: You can only view deposits of users you referred.');
        }
    ...
    {
        $admin = auth('admin')->user();
        $deposit = Deposit::where('id', $id)->where('status', Status::PAYMENT_PENDING)->with('user')->firstOrFail();

        // Verify sub-admin access
        if ($admin->isSubAdmin == 1 && $deposit->user->ref_by != $admin->id) {
            abort(403, 'Unauthorized: You can only approve deposits of users you referred.');
        }
    ...
    public function reject(Request $request)
    {
        ...
        // Verify sub-admin access
        if ($admin->isSubAdmin == 1 && $deposit->user->ref_by != $admin->id) {
            abort(403, 'Unauthorized: You can only reject deposits of users you referred.');
        }
